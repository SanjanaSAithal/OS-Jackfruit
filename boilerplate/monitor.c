 ...
 * in process context (softirq-deferred work via timer_list on modern
 * kernels), and the ioctl paths may sleep during copy_from_user.
 * A mutex is safe here and avoids the restriction that spinlocks
 * cannot be held across code that may sleep.
 * ============================================================== */
...
static void timer_callback(struct timer_list *t)
{
     ...
     * without use-after-free — 'tmp' holds the next pointer before we
...
static long monitor_ioctl(struct file *f, unsigned int cmd, unsigned long arg)
{
    ...
    if (cmd == MONITOR_REGISTER) {
        ...
        if (req.soft_limit_bytes > req.hard_limit_bytes) {
            printk(KERN_WARNING
                   "[container_monitor] Invalid limits for pid=%d\n", req.pid);
            return -EINVAL;
        }
